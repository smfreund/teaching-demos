...
```{r}
soil.nut<-tidyr::gather(soil, nutrient, value, c(10,11,13))
ggplot(soil.nut) +
...
```{r}
soil.nut2<-tidyr::gather(soil, nutrient, value, c(10,11,12))
ggplot(soil.nut2) +
  geom_point(aes(x=pH, y=value, fill=nutrient), shape=21, color="black", size=4, stroke=1.5)
```
...
Check out the options and play around with different themes until you find some that appeal to you and lend themselves to the type of data visualization and color scales you plan to use. You can also fine-tune pretty much any of the details (gridlines or no? plot borders? line weight of the borders and gridlines? axis label angles?) to your heart's content using the arguments in the **`theme()`** function.
...
To get an idea for how other kinds of plots are built in ggplot, we'll look at two more: histograms and barplots.
...
Here is how to add a density curve to the histogram:
...
And here's how we can simply compare our distributions to a normal distribution:


```{r fig.height=6, fig.width=14}
ggplot(soil.nut) +
  geom_histogram(aes(x=value, y=..density..), color='black', fill='white',bins=15) +
  #geom_density(aes(x=value,color=nutrient)) +
...
Several packages have been created that build off of ggplot's syntax with additional functions. You can find a list of them [here](http://www.ggplot2-exts.org/gallery/). We'll do an example using ggmap, a popular package for mapping.
...
This section might mention visreg, corrplot/ggcorrplot, maptree, rpart.plot, others specific to certain analytical methods
