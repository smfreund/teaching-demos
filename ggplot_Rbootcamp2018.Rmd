...
The problem here is that K occurs at natural concentrations that differ from the other nutrients by an order of magnitude, so it's difficult to examine them all on the same scale. While we're at it, let's also make sure that we specify the response units on the y-axis. 
...
* Maybe I should also throw in `gridExtra` since it's sweet
